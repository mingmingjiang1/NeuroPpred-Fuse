...
class Relif():
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.max_iter):
            sample_seed = np.random.randint(0, m-1)
            ...
            for i in range(n):
                ...
                if isinstance(sample_x[i], str):
                    ...
                    near_hit_sum += pow(sample_x[i] - near_hit[i], 2)
                    near_miss_sum += pow(sample_x[i] - near_miss[i], 2)
                    
                weight[i] += near_miss_sum - near_hit_sum

        weight = weight / self.max_iter
        self.chosen_features = np.argsort(-np.array(weight))
        
        #for i, w in enumerate(np.argsort(-np.array(weight))):
    ...
    def transform(self, X):
        ...
        return X[:, self.chosen_features],self.chosen_features[:self.tao]
